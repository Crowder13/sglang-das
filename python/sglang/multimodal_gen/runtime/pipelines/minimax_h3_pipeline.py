...
class MiniMaxH3Pipeline(LoRAPipeline, ComposedPipelineBase):
    ...
    def _load_config(self):
        ...
        if self.server_args.model_subfolder is not None:
            load_path = os.path.join(self.model_path, self.server_args.model_subfolder)
        self.model_path = load_path
        if os.path.isfile(os.path.join(self.model_path, "model_index.json")):
            model_index = verify_model_config_and_directory(self.model_path)
        else:
