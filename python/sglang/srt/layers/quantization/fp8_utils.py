...
def apply_fp8_linear(
    ...
    if not isinstance(input, tuple):
        ...
        if compressed_tensor_quant:
            ...
            # For static per-tensor activation scales when using inductor
            # compiler, use pure PyTorch ops instead of the opaque sgl_kernel
            # quant kernel. Inductor fuses these with surrounding ops (RMSNorm,
            # residual add), eliminating a separate kernel launch per linear
            # layer. weight_scale shape does not matter here -- it is only used
            # in the GEMM epilogue, not in the activation quant fusion. Only
            # activates when cuda_graph_config[prefill].tc_compiler=inductor;
            # eager PCG and decode both use the faster custom kernel.
            if (
