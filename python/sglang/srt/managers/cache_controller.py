...
@dataclass
class PrefetchAck:
    """Cross-rank progress notification for one L3 prefetch operation."""
...
class HiCacheController:
    ...
    def attach_storage_backend(
        ...
        try:
            ...
            # from other collectives and consistent across CPxTPxPP participants.
    ...
    def _page_transfer(self, operation: PrefetchOperation) -> None:
        ...
        for i in range(0, len(operation.hash_value), self.storage_batch_size):
            # Every rank must enqueue the same number of progress ACKs so the
            # completion all-reduces stay ordered, even after a local failure.
            ...
            if all_success:
                ...
                count = self.page_get_func(
                    operation, batch_hashes, batch_host_indices, extra_info
                )
                if count != len(batch_hashes):
                    all_success = False
                completed_tokens += count * self.page_size
    ...
    def prefetch_sync_thread_func(self):
        """Synchronize prefetch progress across TP/CP/PP ranks."""
        while not self.storage_stop_event.is_set():
            try:
